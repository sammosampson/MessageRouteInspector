...
Scenario: logging the fact that a command is being processed 
	...
	Then graphQL should be posted of '{"query":"mutation logCommandProcessing { logCommandProcessing(name: "TestCommand", machine: "TestMachine", thread: 1, createdOn: "635783904000000000") }","variables":{}}'
...
Scenario: logging the fact that a event is being processed 
	...
	Then graphQL should be posted of '{"query":"mutation logEventProcessing { logEventProcessing(name: "TestEvent", machine: "TestMachine", thread: 1, createdOn: "635783904000000000") }","variables":{}}'
	And it should be posted to 'http://test.com'
...
Scenario: logging the fact that a message has failed in processing
	...
	Then graphQL should be posted of '{"query":"mutation logMessageProcessingFailure { logMessageProcessingFailure(name: "TestFailure", machine: "TestMachine", thread: 1, createdOn: "635783904000000000") }","variables":{}}'
	And it should be posted to 'http://test.com'
...
Scenario: logging the fact that a message has been processed
	...
	Then graphQL should be posted of '{"query":"mutation logMessageProcessed { logMessageProcessed(machine: "TestMachine", thread: 1) }","variables":{}}'
	And it should be posted to 'http://test.com'
